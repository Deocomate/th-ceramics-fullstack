...
class DenVuonGomSuCtService
{
    ...
    public function findById(int $id): DenVuonGomSuCt { return DenVuonGomSuCt::findOrFail($id); }
    public function create(array $data): DenVuonGomSuCt {
        return DB::transaction(function () use ($data) {
            $fillable = [
                'name' => $data['name'], 'size' => $data['size'] ?? null,
            ...
            if (!empty($data['images']) && is_array($data['images'])) {
                foreach ($data['images'] as $file) {
                    if ($file instanceof UploadedFile) $images[] = FileUploadHelper::upload($file, 'den_vuon_gom_su_ct/images');
                }
            }
    ...
    public function update(int $id, array $data): DenVuonGomSuCt {
        ...
        return DB::transaction(function () use ($model, $data) {
            $fillable = [
                'name' => $data['name'], 'size' => $data['size'] ?? $model->size,
            ...
            if (!empty($data['new_images']) && is_array($data['new_images'])) {
                ...
                foreach ($data['new_images'] as $file) {
                    if ($file instanceof UploadedFile) $currentImages[] = FileUploadHelper::upload($file, 'den_vuon_gom_su_ct/images');
                }
    ...
    public function toggleStatus(int $id, int $status): void {
        ...
        if ($status === 1) $model->phanLoais()->update(['is_delete' => 1]);
    }
