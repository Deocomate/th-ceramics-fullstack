...
class LanCanGomSuCtService
{
    ...
    public function findById(int $id): LanCanGomSuCt { return LanCanGomSuCt::findOrFail($id); }
        ...
        return DB::transaction(function () use ($data) {
            ...
            if (!empty($data['images']) && is_array($data['images'])) {
                foreach ($data['images'] as $file) {
                    if ($file instanceof UploadedFile) $images[] = FileUploadHelper::upload($file, 'lan_can_gom_su_ct/images');
                }
            }
        ...
        return DB::transaction(function () use ($model, $data) {
            ...
            if (!empty($data['new_images']) && is_array($data['new_images'])) {
                ...
                foreach ($data['new_images'] as $file) {
                    if ($file instanceof UploadedFile) $currentImages[] = FileUploadHelper::upload($file, 'lan_can_gom_su_ct/images');
                }
