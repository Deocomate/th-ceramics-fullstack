<x-layouts.client title="Gạch Hoa Thông Gió" data-page="products" main-class="bg-background-secondary pb-14 md:pb-20" :hide-newsletter="true">
...
<section class="hidden md:flex relative w-full h-[180px] md:h-[210px] items-center justify-center overflow-hidden">
    <div class="absolute inset-0 z-0">
        <img src="{{ !empty($product->images) && is_array($product->images) && isset($product->images[0]) ? Storage::url($product->images[0]) : asset('assets/images/detail-banner.png') }}" alt="{{ $product->name ?? 'Gạch Hoa Thông Gió' }} Banner" class="w-full h-full object-cover" />
    </div>
    <div class="relative z-10 text-center text-white px-4 pt-4">
        <h1 class="text-2xl md:text-3xl font-bold mb-2.5 uppercase">
            {{ $product->name ?? 'Gạch Hoa Thông Gió' }}
        </h1>
        <p class="text-xs md:text-sm text-white/80">
            ...
            <a href="{{ route('client.products.gach-hoa-thong-gio.index') }}" class="hover:text-white transition-colors">Gạch Hoa Thông Gió</a>
        </p>
    </div>
</section>
...
<div class="flex flex-col md:block">
    <div class="order-2 md:order-none">
        <x-products.quantity-calculator image="{{ asset('assets/images/gtt-size.png') }}" />
    </div>
