              ...
              <article class="flex-shrink-0 w-full snap-start pb-1" data-product-slide>
                <div class="grid grid-cols-2 gap-4">
                  @foreach($chunk as $product)
                    <a href="{{ route('client.products.phu-kien-ngoi.bo-noc-chu-van.detail', $product->phu_kien_ngoi_ct_id) }}" class="flex flex-col group cursor-pointer">
                      ...
                      <p class="text-gray-500 text-[12px] leading-[20px]">MSP: PKN-CV{{ $product->phu_kien_ngoi_ct_id }}</p>
          ...
          <div class="slider-pages-container relative">
            @foreach($boNocChuVanProducts->chunk(4) as $index => $chunk)
              <div class="grid grid-cols-2 gap-x-8 lg:gap-x-16 gap-y-10 lg:gap-y-12 mb-10 {{ $index === 0 ? '' : 'hidden' }}" data-slider-page="{{ $index }}">
                @foreach($chunk as $product)
                  <div class="flex flex-col group cursor-pointer" onclick="window.location.href = '{{ route('client.products.phu-kien-ngoi.bo-noc-chu-van.detail', $product->phu_kien_ngoi_ct_id) }}'">
                    ...
                    <p class="text-gray-500 text-[12px] lg:text-[13px] mb-1">MSP: PKN-CV{{ $product->phu_kien_ngoi_ct_id }}</p>
