  ...
  <div class="bg-[#EBCEC1] opacity-80 p-6 lg:p-10" style="margin-right: max(0px, calc((100% - 1320px) / 2))"
    ...
    <div class="flex flex-col lg:flex-row gap-8 lg:gap-16 max-w-[1320px] ml-auto">
      ...
      <div class="w-full lg:w-[45%] ml-0 lg:ml-[10%] flex flex-col justify-stretch">
        @php
          $featured = $ngoiBoNocProducts->first();
      ...
      <div class="w-full lg:w-[55%] flex flex-col justify-between">
        <div class="grid grid-cols-2 gap-x-4 md:gap-x-8 lg:gap-x-16 gap-y-6 md:gap-y-10 lg:gap-y-12 mb-6 md:mb-10">
          @foreach($ngoiBoNocProducts->take(4) as $product)
            @php
            ...
            <div class="flex flex-col group cursor-pointer" onclick="window.location.href = '{{ route('client.products.phu-kien-ngoi.detail', $product->ngoi_bo_noc_ct_id) }}'">
              ...
              <h3
                ...
                {{ $product->name }}</h3>
              <p class="text-gray-500 text-[12px] lg:text-[13px] mb-1">MSP: {{ $product->ngoi_bo_noc_ct_id }}</p>
