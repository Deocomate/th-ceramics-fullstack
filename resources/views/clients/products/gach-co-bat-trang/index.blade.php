<x-client.layouts.main title="Gạch Cổ Bát Tràng" data-page="products" main-class="bg-background-secondary"
    ...
    @php
        ...
        $sectionPayload = function (?array $stored, array $fallback) {
            ...
            $colors = is_array($stored['colors'] ?? null)
                ? array_values($stored['colors'])
                : ['#A98467', '#B22222', '#5D5FEF'];
            ...
            return [
                ...
                'description' =>
                    $stored['description'] ??
                    'Sự không đồng màu này không phải là lỗi kỹ thuật, mà chính là "chứng chỉ" cho nghệ thuật chế tác thủ công. Chính sự biến thiên của lửa trong lò bầu, lò ếch ngày xưa đã tạo nên vẻ đẹp "vạn biến" mà các dòng gạch công nghiệp hiện đại khó lòng mô phỏng được.',
    ...
    <section class="relative w-full h-[530px] md:h-[720px] lg:h-[840px] overflow-hidden flex items-start md:mb-[40px]">
        <div class="absolute inset-0 z-0">
            <img src="{{ $assetUrl($config->thumbnail_main, 'assets/images/gach-co-banner.png') }}"
                alt="Gạch Cổ Bát Tràng Banner" class="w-full h-full object-cover" />
        ...
        <section>
            @forelse($sections as $section)
                @if ($section['products']->isNotEmpty())
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 {{ $section['reverse'] ? 'gap-x-12 lg:gap-x-24' : 'gap-x-24 lg:gap-x-36' }} gap-y-[30px] md:gap-y-12 items-start mb-[40px] md:mb-32"
                        data-aos="fade-up">
                        @if ($section['reverse'])
                            <x-client.products.gach-co-bat-trang.product-info-grid :title="$payload['title']" :subtitle="$payload['subtitle']"
                                :description="$payload['description']" :colors="$payload['colors']" wrapper-class="order-2 lg:order-1"
                                title-class="uppercase" :products="$section['products']" />
                        @endif

                        <div class="relative {{ $section['reverse'] ? 'order-1 lg:order-2' : '' }}">
                            <div class="relative mb-[10px] md:mb-6">
                                <img src="{{ $assetUrl($heroImage, $section['fallbacks'][0]) }}"
                                    alt="{{ $section['label'] }}"
                                    class="w-full aspect-[1.1/1] h-auto object-cover rounded-sm shadow-md" />
                                <img src="{{ asset($section['ellipse']) }}" alt=""
                                    class="hidden md:block absolute top-1/2 -translate-y-1/2 {{ $section['reverse'] ? '-left-12 md:-left-20' : '-right-12 md:-right-20' }} max-w-[220px] aspect-square object-contain drop-shadow-2xl z-10"
                            ...
                            <div class="grid grid-cols-3 gap-[10px] md:gap-7">
                                @foreach ($thumbImages as $image)
                                    <div
                                        ...
                                        <img src="{{ $assetUrl($image, $section['fallbacks'][0]) }}"
                                            alt="{{ $section['label'] }}" class="w-full h-full object-cover" />
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        @unless ($section['reverse'])
                            <x-client.products.gach-co-bat-trang.product-info-grid :title="$payload['title']" :subtitle="$payload['subtitle']"
                                :description="$payload['description']" :colors="$payload['colors']" :products="$section['products']" />
                        @endunless

                        <div class="lg:col-span-2 order-3" data-aos="fade-up">
                            <div class="flex items-center justify-between mb-6">
                                <p class="text-primary font-bold uppercase tracking-[0.06em] text-[13px]">Danh sách sản
                                    phẩm {{ $section['label'] }}</p>
                                <div class="hidden md:flex items-center gap-3">
                                    <button type="button"
                                        class="{{ $sliderPrefix }}-prev product-list-nav-btn w-10 h-10 border-2 border-black/20 rounded-full flex items-center justify-center text-black/40 hover:border-black hover:text-black transition-all">
                                    ...
                                    <button type="button"
                                        class="{{ $sliderPrefix }}-next product-list-nav-btn w-10 h-10 border-2 border-black/20 rounded-full flex items-center justify-center text-black/40 hover:border-black hover:text-black transition-all">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 5l7 7-7 7"></path>
                                        </svg>
                                    ...
                                    @foreach ($section['products'] as $product)
                                        ...
                                        <div class="swiper-slide !h-auto">
                                            <x-client.shared.product-card
                                                href="{{ route('client.products.gach-co-bat-trang.detail', $product->gach_co_bat_trang_ct_id) }}"
                                                image="{{ $assetUrl($productImage, $section['fallbacks'][0]) }}"
                                                title="{{ $product->name }}"
                            ...
                            <div
                                class="swiper-pagination {{ $sliderPrefix }}-pagination product-list-pagination md:!hidden mt-10 relative flex justify-center w-full">
                            </div>
                        </div>
                    </div>
                @endif
    ...
    <x-client.shared.recommendations :related-products="$recommendationProducts" route-name="client.products.gach-co-bat-trang.detail"
        pk-field="gach_co_bat_trang_ct_id" product-type="gach_co_bat_trang_ct" />
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                initSectionProductSlider(".gach-bat-products-slider", ".gach-bat-products-next",
                    ".gach-bat-products-prev", ".gach-bat-products-pagination");
                initSectionProductSlider(".gach-that-products-slider", ".gach-that-products-next",
                    ".gach-that-products-prev", ".gach-that-products-pagination");
                initSectionProductSlider(".gach-the-products-slider", ".gach-the-products-next",
                    ".gach-the-products-prev", ".gach-the-products-pagination");
