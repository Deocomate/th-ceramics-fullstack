<x-client.layouts.main title="Đăng ký tài khoản" main-class="bg-[#F8F5F0] min-h-screen flex flex-col">
    <!-- Phần Main Content Đăng Ký -->
    <section class="relative flex-grow flex items-center justify-center py-12 lg:py-20 overflow-hidden">
        ...
        <!-- Form Container -->
        <div class="relative z-10 w-[90%] max-w-[1000px] bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] overflow-hidden flex flex-col lg:flex-row-reverse"
            data-aos="fade-up">

            <!-- Cột phải (đảo ngược so với login): Hình ảnh Branding (Chỉ hiện trên Desktop) -->
            <div class="hidden lg:flex lg:w-1/2 relative bg-primary items-center justify-center p-12 overflow-hidden">
                <!-- Thay đổi ảnh hoặc giữ nguyên, thêm hiệu ứng -->
                ...
                <div class="relative z-10 text-center flex flex-col items-center">
                    ...
                    <h2 class="text-white text-3xl font-bold uppercase tracking-wider mb-4 font-archivo">Gia nhập cộng
                        đồng</h2>
                    <p class="text-white/90 text-sm leading-relaxed font-roboto max-w-sm">
                        Khám phá di sản gốm sứ Bát Tràng và bắt đầu hành trình kiến tạo những công trình mang đậm dấu ấn
                        kiến trúc Việt cùng TH Ceramics.
                    </p>
                </div>
            </div>

            <!-- Cột trái: Form Đăng ký -->
            <div class="w-full lg:w-1/2 p-8 lg:px-12 lg:py-10 flex flex-col justify-center overflow-y-auto">
                ...
                <div class="text-center lg:text-left mb-8">
                    <h1 class="text-2xl lg:text-3xl font-bold uppercase text-primary mb-2 font-archivo">Đăng ký tài
                        khoản</h1>
                    <p class="text-gray-500 text-sm font-roboto">Điền thông tin để tạo tài khoản mới</p>
                </div>
                ...
                <form action="{{ route('client.auth.register.post') }}" method="POST" class="space-y-4 font-roboto">
                    @csrf

                    <!-- Name Input -->
                    <div>
                        <label for="name" class="block text-sm font-semibold text-primary mb-1">Họ và tên <span
                                class="text-red-500">*</span></label>
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required
                            autofocus
                            class="w-full px-4 py-3 border @error('name') border-red-500 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-1 focus:ring-secondary focus:border-secondary transition-colors"
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-semibold text-primary mb-1">Xác
                            nhận mật khẩu <span class="text-red-500">*</span></label>
                        <div class="relative">
                    ...
                    <!-- Submit Button -->
                    <button type="submit"
                        class="w-full mt-4 bg-secondary text-white font-bold text-sm uppercase tracking-wider py-3.5 rounded-md hover:bg-[#a65b00] transition-colors shadow-md">
                        Đăng ký ngay
                    </button>
                </form>
                ...
                <!-- Login Link -->
                <p class="text-center text-sm text-gray-600 font-roboto">
                    Đã có tài khoản?
                    <a href="{{ route('client.auth.login') }}"
                        class="font-bold text-secondary hover:underline transition-all">Đăng nhập ngay</a>
                </p>
