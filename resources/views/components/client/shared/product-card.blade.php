<div {{ $attributes->merge(['class' => 'flex flex-col group cursor-pointer']) }}>
    ...
    @if ($price || $code || $shouldShowAddToCart)
        <div @class([
            ...
            'justify-end' => $shouldShowAddToCart && !$price && !$code && !$addToCartAlignStart,
            'justify-start' => $shouldShowAddToCart && !$price && !$code && $addToCartAlignStart,
        ])>
            @if ($code || $price)
                <div class="flex flex-col gap-0.5 min-w-0 flex-1">
                    @if ($code)
                        <p class="text-[#3C4043] font-light text-[13px] leading-tight">
                            {{ $code }}
                        </p>
                    @endif
                    @if ($price)
                        <p class="text-secondary font-bold text-[14px] md:text-[15px] leading-tight min-w-0 truncate">
                            {{ $price }}
                        </p>
                    @endif
                </div>
