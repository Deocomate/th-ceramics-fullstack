...
@php
    $resolvedRate = $rate;
    if ($resolvedRate === null && ! empty($dinhMuc)) {
        $firstDinhMuc = is_object($dinhMuc) && method_exists($dinhMuc, 'first')
            ? $dinhMuc->first()
            : (is_array($dinhMuc) ? ($dinhMuc[0] ?? null) : null);
        $resolvedRate = $firstDinhMuc?->value ?? null;
    }
            ...
            <div class="flex flex-col mt-4 md:mt-0">
                ...
                <div class="space-y-4 md:space-y-6 w-[90%] md:w-full mx-auto md:mx-0" id="calculator-fields-container">
                    <div class="flex flex-row md:flex-row gap-4 md:gap-6 items-end justify-center md:justify-start area-block" data-area-block>
                        <div class="w-[111px] md:w-[120px] shrink-0 pb-2 md:pb-1 text-left flex flex-col items-start">
                            ...
                            <button type="button" data-remove-area class="text-[8px] md:text-[14px] text-[#C76E00]/70 md:text-secondary underline text-start font-normal md:font-medium opacity-80 hover:opacity-100 transition-opacity mt-0.5 md:not-italic hidden">Loại bỏ</button>
                        </div>
                        <div class="flex-1 max-w-[250px] md:max-w-none w-full grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-6 items-end">
                            <hr class="hidden md:block md:col-span-1 border-transparent" />
                            <div class="flex flex-col gap-1 md:gap-2 md:col-span-2">
                                ...
                                <div class="relative">
                                    <input type="number" step="0.01" min="0" placeholder="00" data-input-length
                                        class="w-full h-[30px] md:h-[45px] p-2 md:p-3 text-[12px] md:text-base border border-[#DDD6D0] md:border-black/10 rounded-[1px] md:rounded-sm bg-transparent text-left pr-6 md:pr-8 focus:border-secondary outline-none transition-colors text-[#2E2F2A] md:text-primary placeholder:text-[#9CA3AF]" />
                            ...
                            <div class="flex flex-col gap-1 md:gap-2 md:col-span-2">
                                ...
                                <div class="relative">
                                    <input type="number" step="0.01" min="0" placeholder="00" data-input-width
                                        class="w-full h-[30px] md:h-[45px] p-2 md:p-3 text-[12px] md:text-base border border-[#DDD6D0] md:border-black/10 rounded-[1px] md:rounded-sm bg-transparent text-left pr-6 md:pr-8 focus:border-secondary outline-none transition-colors text-[#2E2F2A] md:text-primary placeholder:text-[#9CA3AF]" />
                                    <span class="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 text-[10px] md:text-[12px] text-black/40 md:text-primary/40 leading-[15px]">m</span>
                                </div>
                            </div>
                ...
                <div class="space-y-4 md:space-y-6 w-[90%] md:w-full mx-auto md:mx-0 mt-4">
                    ...
                    <div class="w-full pt-4 md:pt-0 pb-6 md:pb-0">
                        <button type="button" id="btn-calculate-quantity"
                            class="w-full py-[16px] md:py-4 bg-[#C16A00] hover:bg-secondary text-white text-[14px] font-bold uppercase tracking-[0.42px] md:tracking-widest rounded-[2px] md:rounded-sm shadow-md transition-all duration-300 leading-[20px]">TÍNH
                        ...
                        <div class="flex flex-col md:grid md:grid-cols-5 gap-y-2 md:gap-y-4 items-start w-full">
                            <div class="flex justify-between items-center w-full md:contents">
                                <div class="md:col-span-4">
                                    <h4 class="font-semibold text-[#2E2F2A] md:text-primary uppercase text-[16px] md:text-xl leading-[28px] md:leading-normal">
                                        KẾT QUẢ
                                    </h4>
                                </div>
                                <div class="text-right">
                                    <span id="output-total-area" class="text-[16px] md:text-2xl font-bold text-[#2E2F2A] md:text-primary leading-[20px] md:leading-none">
                                        0 m²
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-row items-end justify-start w-full md:contents pt-1 md:pt-0 relative">
                                <div class="md:col-span-3 pr-2 md:pr-0">
                                    <p class="text-[12px] md:text-[14px] italic text-[#2E2F2A] md:text-primary font-light md:font-light leading-[18px] max-w-[132px] md:max-w-none">
                                        Diện tích được làm tròn lên đến m² gần nhất.
                                    </p>
                                </div>
                            ...
                            <div class="flex justify-between items-center w-full md:contents">
                                <div class="md:col-span-3 py-1 text-left w-[40%] md:w-auto">
                                    <span class="text-[#2E2F2A] md:text-primary text-[16px] md:text-base leading-[24px]">Gạch cần dùng</span>
                                </div>
                                <div class="text-center py-1 w-[30%] md:w-auto">
                                    <span id="output-rate" class="text-[#2E2F2A] md:text-primary text-[14px] md:text-base leading-[16px]">
                                        {{ $activeRate }} viên/m²
                                    </span>
                                </div>
                                <div class="text-right py-1 w-[30%] md:w-auto flex justify-end">
                                    <span id="output-total-bricks" class="text-[#2E2F2A] md:text-primary font-bold text-[14px] md:text-base leading-[16px]">
                                        0 viên
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="space-y-4 pt-4 border-t border-black/10 border-solid mt-2 md:mt-0">
                        ...
                        <div class="pl-8 space-y-3 hidden peer-checked:block" id="loss-rate-options">
