<x-admin.layout.app title="Gạch Cổ Bát Tràng" breadcrumb="Admin › Sản Phẩm › Gạch Cổ Bát Tràng">
        ...
        <form method="POST" action="{{ route('admin.gach-co-bat-trang.update') }}" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="aspect-video w-full rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex items-center justify-center overflow-hidden relative group">
                            <img id="preview-main" src="{{ asset('storage/' . $gachCoBatTrang->thumbnail_main) }}" onerror="this.src='https://placehold.co/600x400?text=Chua+co+anh'" class="w-full h-full object-contain" alt="Ảnh chính">
                ...
                <div class="flex flex-col h-full border border-gray-200 rounded-xl p-4 bg-gray-50/50">
                   ...
                   <div class="h-[420px] bg-white border border-gray-200 rounded-xl p-4 overflow-y-auto shadow-inner flex flex-col">
                        <div id="multiple-preview-container" class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                            <div id="empty-preview-state" class="col-span-full h-full min-h-[340px] flex flex-col items-center justify-center text-center text-gray-400 text-xs font-medium gap-2">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            ...
            @if(is_array($gachCoBatTrang->images))
                <span class="text-xs text-gray-500 font-medium">Tổng số: {{ count($gachCoBatTrang->images) }} ảnh</span>
            @endif
        </div>
        <div class="p-6">
            @if(is_array($gachCoBatTrang->images) && count($gachCoBatTrang->images) > 0)
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
                    @foreach($gachCoBatTrang->images as $path)
