<x-admin.layout.app title="Lan Can Gốm Sứ" breadcrumb="Admin › Sản Phẩm › Lan Can Gốm Sứ">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wide">Giá Trị Lan Can Gốm Sứ</h2>
        </div>
        
        <div class="p-6">
            {{-- HIỂN THỊ CẢNH BÁO LỖI TỔNG QUAN GIỐNG THÔNG BÁO THÀNH CÔNG --}}
            @if ($errors->any())
            ...
            {{-- FORM THÊM MỚI GIÁ TRỊ (Đã làm sạch sẽ, không hiển thị lỗi đỏ xuống đây nữa) --}}
            <div class="mb-10 p-6 bg-blue-50/30 rounded-xl border border-blue-100 shadow-[0_0_15px_rgba(59,130,246,0.03)]">
                ...
                <!-- Lưu ý: Thay route action tương ứng với từng file (ngoi-am-duong hoặc lan-can-gom-xu) -->
                <form method="POST" action="{{ request()->routeIs('admin.ngoi-am-duong.*') ? route('admin.ngoi-am-duong.gia-tri.store') : route('admin.lan-can-gom-xu.gia-tri.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 flex flex-col gap-5">
                            <div>
                                ...
                                <input type="text" name="title" required placeholder="VD: Kháng nước, chống rêu mốc (Tối đa 50 ký tự)" class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
                            </div>
                            <div class="flex-1 flex flex-col">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Mô tả chi tiết <span class="text-red-500">*</span></label>
                                <textarea name="desscription" required placeholder="Nhập nội dung mô tả chi tiết..." class="w-full flex-1 min-h-[100px] px-4 py-3 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all resize-none"></textarea>
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @forelse($lanCanGomXu->giaTri as $giaTri)
                    <div class="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow bg-white flex flex-col group relative">
                        ...
                        <div class="w-[300px] h-[460px] mx-auto mt-4 relative bg-gray-100 flex-shrink-0 rounded-xl overflow-hidden border border-gray-100">
                            ...
                            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 backdrop-blur-[2px]">
                                <button type="button" 
                                    ...
                                    onclick="openEditModal(this, '{{ route('admin.lan-can-gom-xu.gia-tri.update', $giaTri) }}')"
                                ...
                                <button type="button" 
                                    onclick="openDeleteModal('{{ route('admin.lan-can-gom-xu.gia-tri.destroy', $giaTri) }}')"
                        ...
                        <div class="p-5 w-full flex flex-col flex-1 text-center">
                            {{-- Thêm truncate cho tiêu đề --}}
                            <h4 class="font-bold text-gray-800 text-base mb-2 truncate" title="{{ $giaTri->title }}">{{ $giaTri->title }}</h4>
                            {{-- Thêm break-all để ép chuỗi dính liền bẻ dòng --}}
                            <p class="text-sm text-gray-500 line-clamp-2 leading-relaxed break-all" title="{{ $giaTri->desscription }}">{{ $giaTri->desscription }}</p>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full py-12 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                        <p class="text-sm font-medium text-gray-500">Chưa có giá trị nào được cấu hình.</p>
                    </div>
                @endforelse
            </div>
        </div>
                ...
                <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 flex flex-col gap-5">
                            ...
                            <div class="flex-1 flex flex-col">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Mô tả chi tiết <span class="text-red-500">*</span></label>
                                <textarea id="edit_description" name="desscription" required class="w-full flex-1 min-h-[100px] px-4 py-3 text-sm border rounded-lg border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all resize-none"></textarea>
                            </div>
                        </div>
                    </div>
