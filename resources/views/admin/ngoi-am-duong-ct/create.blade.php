<x-admin.layouts.app title="Thêm Ngói Âm Dương" breadcrumb="Admin › DS Sản phẩm chi tiết › Thêm mới">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.ngoi-am-duong-ct.store') }}" enctype="multipart/form-data" class="p-6">
            ...
            <div class="flex flex-col h-full border rounded-xl p-6 bg-gray-50/50 {{ $errors->has('images') ? 'border-red-300 bg-red-50/30' : 'border-gray-200' }}">
                ...
                <p class="text-xs text-gray-500 mb-4">Chọn 1 hoặc nhiều ảnh chi tiết chung của sản phẩm (Ảnh đầu tiên sẽ làm ảnh bìa).</p>
    ...
    @push('scripts')
        <script>
            ...
            // [QUAN TRỌNG] Lấy data old() nếu validation báo lỗi, hoặc để trống 1 block mặc định
            const oldDes = @json(old('des',[]));
            if (oldDes && oldDes.length > 0) {
                oldDes.forEach(val => {
                    if(val) addDesBlock(val); // render lại giá trị người dùng vừa nhập
                });
            } else {
                addDesBlock('');
            }

            // Upload Nhiều Ảnh logic
            let selectedFiles =[];
