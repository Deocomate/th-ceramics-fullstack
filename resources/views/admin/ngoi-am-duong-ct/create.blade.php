<x-admin.layouts.app title="ThÃªm NgÃ³i Ã‚m DÆ°Æ¡ng" breadcrumb="Admin â€º DS Sáº£n pháº©m chi tiáº¿t â€º ThÃªm má»›i">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.ngoi-am-duong-ct.store') }}" enctype="multipart/form-data" class="p-6">
            @csrf
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="code" value="{{ old('code') }}" required placeholder="VD: NAD-001"
            ...
            <div class="flex flex-col h-full border rounded-xl p-6 bg-gray-50/50 {{ $errors->has('images') ? 'border-red-300 bg-red-50/30' : 'border-gray-200' }}">
                ...
                <p class="text-xs text-gray-500 mb-4">Chá»n 1 hoáº·c nhiá»u áº£nh chi tiáº¿t chung cá»§a sáº£n pháº©m (áº¢nh Ä‘áº§u tiÃªn sáº½ lÃ m áº£nh bÃ¬a).</p>
                <div class="relative mb-4">
                    <input type="file" id="multipleImagesInput" name="images[]" multiple accept="image/*"
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.ngoi-am-duong-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Há»§y</a>
    ...
    @push('scripts')
        <script>
            ...
            // [QUAN TRá»ŒNG] Láº¥y data old() náº¿u validation bÃ¡o lá»—i, hoáº·c Ä‘á»ƒ trá»‘ng 1 block máº·c Ä‘á»‹nh
            const oldDes = @json(old('des',[]));
            if (oldDes && oldDes.length > 0) {
                oldDes.forEach(val => {
                    if(val) addDesBlock(val); // render láº¡i giÃ¡ trá»‹ ngÆ°á»i dÃ¹ng vá»«a nháº­p
                });
            } else {
                addDesBlock('');
            }

            // Upload Nhiá»u áº¢nh logic
            let selectedFiles =[];
