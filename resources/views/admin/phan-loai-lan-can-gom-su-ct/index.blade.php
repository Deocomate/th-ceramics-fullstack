<x-admin.layouts.app title="Phân Loại Lan Can Gốm Sứ" breadcrumb="Admin › DS Sản phẩm chi tiết › Lan Can Gốm Sứ › Phân Loại">
    @if ($errors->any())
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            @if(isset($selectedProduct))
                <a href="{{ route('admin.lan-can-gom-su-ct.index') }}" class="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-[#A31D1D] hover:bg-red-50 transition-colors shadow-sm">
        ...
        <form method="GET" action="{{ route('admin.phan-loai-lan-can-gom-su-ct.index') }}" class="flex items-center gap-2">
            @if(isset($selectedProduct)) <input type="hidden" name="product_id" value="{{ $selectedProduct->lan_can_gom_su_ct_id }}"> @endif
    ...
    <div class="mb-8 p-6 bg-blue-50/40 rounded-xl border border-blue-100 shadow-sm relative overflow-hidden">
        ...
        <form action="{{ route('admin.phan-loai-lan-can-gom-su-ct.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-4 gap-5 items-end">
                <div class="md:col-span-1">
                    ...
                    @if(isset($selectedProduct))
                        <input type="hidden" name="lan_can_gom_su_ct_id" value="{{ $selectedProduct->lan_can_gom_su_ct_id }}">
                    ...
                    @else
                        <select name="lan_can_gom_su_ct_id" required class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] bg-white">
                            <option value="">-- Chọn sản phẩm --</option>
                            @foreach($products as $prod)
                                <option value="{{ $prod->lan_can_gom_su_ct_id }}" {{ old('lan_can_gom_su_ct_id') == $prod->lan_can_gom_su_ct_id ? 'selected' : '' }}>{{ $prod->name }}</option>
                            @endforeach
                        </select>
                    @endif
                </div>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors {{ $item->is_delete ? 'bg-red-50/30' : '' }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                @if(!$item->is_delete)
                                    <button type="button" data-id="{{ $item->phan_loai_lan_can_gom_su_ct_id }}" data-pid="{{ $item->lan_can_gom_su_ct_id }}" data-name="{{ $item->name }}" data-code="{{ $item->code }}" data-price="{{ $item->price }}" onclick="openEditModal(this, '{{ route('admin.phan-loai-lan-can-gom-su-ct.update', $item->phan_loai_lan_can_gom_su_ct_id) }}')" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                                    <button type="button" onclick="openDeleteModal('{{ route('admin.phan-loai-lan-can-gom-su-ct.destroy', $item->phan_loai_lan_can_gom_su_ct_id) }}')" class="p-2 text-red-600 hover:bg-red-50 rounded-lg"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                                @else
                                    <form method="POST" action="{{ route('admin.phan-loai-lan-can-gom-su-ct.restore', $item->phan_loai_lan_can_gom_su_ct_id) }}" class="inline">
                                        @csrf @method('PUT')
            ...
            <form id="editForm" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="hidden" name="lan_can_gom_su_ct_id" id="edit_pid_hidden">
                        <select id="edit_pid" disabled class="w-full px-4 py-2.5 text-sm bg-gray-100 text-gray-500 rounded-lg">
                            @foreach($products as $prod) <option value="{{ $prod->lan_can_gom_su_ct_id }}">{{ $prod->name }}</option> @endforeach
                        </select>
                    </div>
                    <div class="md:col-span-2">
