<x-admin.layouts.app title="Thêm Bài Viết Mới" breadcrumb="Admin › Tin Tức › Viết Bài">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8" x-data="articleBuilder()">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-white flex justify-between items-center rounded-t-xl">
            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wide">Viết bài Tạp Chí / Tin Tức</h2>
        ...
        <form id="articleForm" method="POST" action="{{ route('admin.tin-tuc.store') }}" enctype="multipart/form-data" class="p-6">
            @csrf
            <input type="hidden" name="trang_thai" id="input_trang_thai" value="draft">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-10">
                <div class="lg:col-span-1">
                    ...
                    <div class="w-full aspect-[4/3] rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex items-center justify-center overflow-hidden relative group hover:bg-gray-100 transition-colors">
                        <img id="preview-thumbnail" src="https://placehold.co/400x300?text=Chon+Anh" class="w-full h-full object-contain">
                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                            <span class="text-white text-xs font-medium px-3 py-1.5 bg-black/50 rounded-lg">Tải ảnh lên</span>
                        </div>
                        <input type="file" name="anh_dai_dien" required accept="image/*" onchange="document.getElementById('preview-thumbnail').src = window.URL.createObjectURL(this.files[0])" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                    </div>
                </div>
                ...
                <div class="lg:col-span-3 space-y-5">
                    <div>
                        ...
                        <input type="text" name="tieu_de" value="{{ old('tieu_de') }}" required placeholder="VD: Giới thiệu mẫu rèm cửa phong cách Minimalist 2024" class="w-full px-4 py-2.5 text-lg font-bold border rounded-lg focus:border-[#A31D1D] outline-none">
                    </div>
                        ...
                        <div>
                            ...
                            <select name="danh_muc_tin_tuc_id" required class="w-full px-4 py-2.5 text-sm border rounded-lg focus:border-[#A31D1D] outline-none bg-white">
                                ...
                                @foreach($danhMucs as $dm)
                                    <option value="{{ $dm->danh_muc_tin_tuc_id }}">{{ $dm->ten_danh_muc }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Thể loại / Tags (Ngăn cách bằng dấu phẩy)</label>
                            <input type="text" name="the_loai" value="{{ old('the_loai') }}" placeholder="VD: Nội thất, Minimalist, 2024" class="w-full px-4 py-2.5 text-sm border rounded-lg focus:border-[#A31D1D] outline-none">
                        </div>
                    </div>
                    <div>
                        ...
                        <textarea name="mo_ta_ngan" required rows="3" class="w-full px-4 py-2.5 text-sm border rounded-lg focus:border-[#A31D1D] outline-none resize-none"></textarea>
                    </div>
                </div>
            </div>
                        ...
                        <template x-if="block.type === 'image_metadata'">
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-4">
                                <div class="space-y-4">
                                    ...
                                    <div class="h-48 rounded-lg border-2 border-dashed border-gray-300 relative overflow-hidden bg-white">
