<x-admin.layouts.app title="Giải Thưởng & Thành Tựu" breadcrumb="Admin › Cấu Hình Chung › Giải Thưởng & Thành Tựu">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wide">Danh Sách Giải Thưởng & Thành Tựu</h2>
        </div>
        <div class="p-6">
            @if ($errors->any())
            ...
            <div class="mb-10 p-6 bg-blue-50/30 rounded-xl border border-blue-100 shadow-[0_0_15px_rgba(59,130,246,0.03)]">
                ...
                <form method="POST" action="{{ route('admin.giai-thuong-thanh-tuu.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div class="lg:col-span-1">
                            ...
                            <div class="w-full h-[240px] mx-auto rounded-xl border-2 border-dashed border-blue-300 bg-white flex items-center justify-center overflow-hidden relative group hover:bg-blue-50/50 transition-colors">
                                ...
                                <input type="file" name="image" accept="image/*" required onchange="previewImage(event, 'preview-new-giaithuong')" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                            </div>
                        </div>
                        <div class="lg:col-span-2 flex flex-col gap-5">
                            <div class="flex-1 flex flex-col">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Mô tả (Năm, Tên giải thưởng...) <span class="text-red-500">*</span></label>
                                <textarea name="des" required placeholder="Nhập nội dung mô tả chi tiết..." class="w-full flex-1 min-h-[150px] px-4 py-3 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all resize-none">{{ old('des') }}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                @forelse($giaiThuong as $item)
                    ...
                    <div class="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow bg-white flex flex-col h-full group relative">
                        ...
                        <div class="w-full h-[220px] relative bg-gray-100 flex-shrink-0 border-b border-gray-100">
                            ...
                            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 backdrop-blur-[2px]">
                                <button type="button" 
                                    ...
                                    onclick="openEditModal(this, '{{ route('admin.giai-thuong-thanh-tuu.update', $item->giai_thuong_thanh_tuu_id) }}')"
                                ...
                                <button type="button" 
                                    onclick="openDeleteModal('{{ route('admin.giai-thuong-thanh-tuu.destroy', $item->giai_thuong_thanh_tuu_id) }}')"
                        ...
                        <div class="p-5 w-full flex flex-col flex-1">
                            <p class="text-sm text-gray-700 leading-relaxed text-justify line-clamp-4">{{ $item->des }}</p>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full py-12 text-center border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
                        <p class="text-sm font-medium text-gray-500">Chưa có giải thưởng nào được thêm.</p>
                    </div>
                @endforelse
            </div>
        </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 flex flex-col gap-5">
                            <div class="flex-1 flex flex-col">
                                ...
                                <textarea id="edit_description" name="des" required class="w-full flex-1 min-h-[150px] px-4 py-3 text-sm border rounded-lg border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all resize-none"></textarea>
                            </div>
                        </div>
                    </div>
