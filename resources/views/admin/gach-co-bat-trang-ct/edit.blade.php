...
<x-admin.layouts.app title="Cáº­p nháº­t Gáº¡ch Cá»• BÃ¡t TrÃ ng" breadcrumb="Admin â€º DS Sáº£n pháº©m chi tiáº¿t â€º Chá»‰nh sá»­a">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.gach-co-bat-trang-ct.update', $product->gach_co_bat_trang_ct_id) }}" enctype="multipart/form-data" class="p-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div>
                        ...
                        <select name="category_type" required class="w-full px-4 py-2.5 text-sm border rounded-lg focus:ring-1 outline-none transition-all {{ $errors->has('category_type') ? 'border-red-500 focus:border-red-500 focus:ring-red-500 bg-red-50/50' : 'border-gray-300 focus:border-[#A31D1D] focus:ring-[#A31D1D]' }}">
                            <option value="bat" @selected(old('category_type', $product->category_type ?? 'bat') === 'bat')>Gáº¡ch BÃ¡t</option>
                            <option value="that" @selected(old('category_type', $product->category_type) === 'that')>Gáº¡ch Tháº¥t & XÃ¢y</option>
                            <option value="the" @selected(old('category_type', $product->category_type) === 'the')>Gáº¡ch Tháº»</option>
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="dinh_muc" value="{{ old('dinh_muc', $product->dinh_muc) }}" placeholder="VD: 11 viÃªn/m2"
                                class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
                        ...
                        <div>
                            ...
                            <input type="text" name="weight" value="{{ old('weight', $product->weight) }}" placeholder="VD: 1.5 kg/viÃªn"
                                class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.gach-co-bat-trang-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Há»§y bá»</a>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    @foreach($product->images as $path)
                        <div class="relative group aspect-square rounded-lg overflow-hidden border border-gray-200 shadow-sm bg-gray-100">
                            ...
                            @if($loop->first)
                                <div class="absolute top-2 left-2 bg-[#A31D1D] text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm">áº¢nh bÃ¬a</div>
                            @endif
