...
<x-admin.layouts.app title="C?p nh?t G?ch C? Bát Tràng" breadcrumb="Admin › DS S?n ph?m chi ti?t › Ch?nh s?a">
        ...
        <form method="POST" action="{{ route('admin.gach-co-bat-trang-ct.update', $product->gach_co_bat_trang_ct_id) }}" enctype="multipart/form-data" class="p-6">
            ...
            @if ($errors->any())
                <div class="mb-6 flex items-start gap-3 px-4 py-3 rounded text-sm text-red-800 bg-red-50 border border-red-200 shadow-sm">
                    <div>
                        <strong class="font-semibold block mb-1">Vui lòng ki?m tra l?i các thông tin sau:</strong>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Phân lo?i <span class="text-red-500">*</span></label>
                        <select name="category_type" required class="w-full px-4 py-2.5 text-sm border rounded-lg focus:ring-1 outline-none transition-all {{ $errors->has('category_type') ? 'border-red-500 focus:border-red-500 focus:ring-red-500 bg-red-50/50' : 'border-gray-300 focus:border-[#A31D1D] focus:ring-[#A31D1D]' }}">
                            <option value="bat" @selected(old('category_type', $product->category_type ?? 'bat') === 'bat')>G?ch Bát</option>
                            <option value="that" @selected(old('category_type', $product->category_type) === 'that')>G?ch Th?t & Xây</option>
                            <option value="the" @selected(old('category_type', $product->category_type) === 'the')>G?ch Th?</option>
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Ð?nh m?c</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Cân n?ng</label>
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.gach-co-bat-trang-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">H?y b?</a>
