...
<x-admin.layouts.app title="Sản phẩm: Gạch Cổ Bát Tràng" breadcrumb="Admin › DS Sản phẩm chi tiết › Gạch Cổ Bát Tràng">
    ...
    <div class="flex items-center justify-between mb-5">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.gach-co-bat-trang-ct.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-lg transition-colors duration-200" style="background:#A31D1D;" onmouseover="this.style.background='#8A1818'" onmouseout="this.style.background='#A31D1D'">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-gray-100" data-admin-product-sortable data-priority-endpoint="{{ route('admin.products.priority.update', 'gach-co-bat-trang-ct') }}" data-can-reorder="{{ $status === 'active' && $categoryType !== 'all' ? 'true' : 'false' }}" data-category-type="{{ $categoryType }}">
                        ...
                        <tr data-priority-id="{{ $product->gach_co_bat_trang_ct_id }}" class="hover:bg-gray-50/50 transition-colors {{ $product->is_delete ? 'bg-red-50/30' : '' }}">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    @if(!$product->is_delete)
                                        <a href="{{ route('admin.gach-co-bat-trang-ct.create', ['copy_from' => $product->gach_co_bat_trang_ct_id]) }}" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Nhân bản / Sao chép sản phẩm">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                        </a>
                                        <a href="{{ route('admin.gach-co-bat-trang-ct.edit', $product->gach_co_bat_trang_ct_id) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Sửa">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </a>
                                        <button type="button" onclick="openDeleteModal('{{ route('admin.gach-co-bat-trang-ct.destroy', $product->gach_co_bat_trang_ct_id) }}')" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Ẩn/Xóa">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        </button>
                                    @else
                                        <!-- Nút Khôi phục -->
                                        <button type="button" onclick="openRestoreModal('{{ route('admin.gach-co-bat-trang-ct.restore', $product->gach_co_bat_trang_ct_id) }}')" class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-green-700 bg-green-50 border border-green-200 hover:bg-green-100 rounded-lg transition-colors" title="Khôi phục">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">Chưa có sản phẩm nào. Hãy thêm mới ngay!</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
