<x-admin.layout.app title="Màu Sắc Ngói Hài Cổ (SKU)" breadcrumb="Admin › DS Sản phẩm chi tiết › Ngói Hài Cổ › Màu Sắc">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            @if(isset($selectedProduct))
                <a href="{{ route('admin.ngoi-hai-van-mieu-ct.index') }}" class="p-2 bg-white border border-gray-200 rounded-lg text-gray-500 hover:text-[#A31D1D] hover:bg-red-50 transition-colors shadow-sm" title="Quay lại danh sách sản phẩm">
        ...
        <form method="GET" action="{{ route('admin.mau-sac-ngoi-hai-van-mieu-ct.index') }}" class="flex items-center gap-2">
            @if(isset($selectedProduct))
                <input type="hidden" name="product_id" value="{{ $selectedProduct->ngoi_hai_van_mieu_ct_id }}">
            @endif
    ...
    <div class="mb-8 p-6 bg-blue-50/40 rounded-xl border border-blue-100 shadow-[0_0_15px_rgba(59,130,246,0.03)] relative overflow-hidden">
        ...
        <form action="{{ route('admin.mau-sac-ngoi-hai-van-mieu-ct.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="xl:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="md:col-span-2">
                        ...
                        @if(isset($selectedProduct))
                            ...
                            <input type="hidden" name="ngoi_hai_van_mieu_ct_id" value="{{ $selectedProduct->ngoi_hai_van_mieu_ct_id }}">
                        ...
                        @else
                            ...
                            <select name="ngoi_hai_van_mieu_ct_id" required class="w-full px-4 py-2.5 text-sm font-medium border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none bg-white transition-all">
                                <option value="">-- Chọn sản phẩm cha --</option>
                                @foreach($products as $prod)
                                    <option value="{{ $prod->ngoi_hai_van_mieu_ct_id }}" {{ old('ngoi_hai_van_mieu_ct_id') == $prod->ngoi_hai_van_mieu_ct_id ? 'selected' : '' }}>
                                        {{ $prod->name }}
                                    </option>
                                @endforeach
                            </select>
                                ...
                                <div class="flex justify-end gap-2">
                                    @if(!$item->is_delete)
                                        <button type="button" title="Sửa màu sắc"
                                            data-id="{{ $item->mau_sac_ngoi_hai_van_mieu_ct_id }}" 
                                            data-pid="{{ $item->ngoi_hai_van_mieu_ct_id }}" 
                                            data-name="{{ $item->name }}" 
                                            data-code="{{ $item->code }}" 
                                            data-price="{{ $item->price }}" 
                                            data-img="{{ asset('storage/' . $item->image) }}"
                                            onclick="openEditModal(this, '{{ route('admin.mau-sac-ngoi-hai-van-mieu-ct.update', $item->mau_sac_ngoi_hai_van_mieu_ct_id) }}')" 
                                            class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        </button>
                                        <button type="button" title="Ẩn màu sắc"
                                            onclick="openDeleteModal('{{ route('admin.mau-sac-ngoi-hai-van-mieu-ct.destroy', $item->mau_sac_ngoi_hai_van_mieu_ct_id) }}')" 
                                            class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        </button>
                                    @else
                                        <form method="POST" action="{{ route('admin.mau-sac-ngoi-hai-van-mieu-ct.restore', $item->mau_sac_ngoi_hai_van_mieu_ct_id) }}" class="inline">
                                            @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2 space-y-4">
                        <div>
                            ...
                            <input type="hidden" name="ngoi_hai_van_mieu_ct_id" id="edit_pid_hidden">
                            <select id="edit_pid" disabled class="w-full px-4 py-2.5 text-sm font-medium border rounded-lg outline-none bg-gray-100 text-gray-500 cursor-not-allowed">
                                @foreach($products as $prod) 
                                    <option value="{{ $prod->ngoi_hai_van_mieu_ct_id }}">{{ $prod->name }}</option> 
                                @endforeach
                            </select>
                        </div>
                        <div>
