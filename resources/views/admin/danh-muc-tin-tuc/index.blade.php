<x-admin.layouts.app title="Danh Mục Tin Tức" breadcrumb="Admin › Cấu hình trang đơn › Danh Mục Tin Tức">
    @if ($errors->any())
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-sm font-semibold text-gray-700">Quản lý Danh Mục Tin Tức</h2>
        </div>
        <form method="GET" action="{{ route('admin.danh-muc-tin-tuc.index') }}" class="flex items-center gap-2">
    ...
    <div class="mb-8 p-6 bg-blue-50/40 rounded-xl border border-blue-100 shadow-sm relative overflow-hidden">
        ...
        <form action="{{ route('admin.danh-muc-tin-tuc.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-4 gap-5 items-end">
                <div class="md:col-span-3">
                    ...
                    <input type="text" name="ten_danh_muc" value="{{ old('ten_danh_muc') }}" required placeholder="VD: Kiến trúc nội thất" class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D]">
                </div>
                <div>
        ...
        <table class="w-full text-sm text-left">
            <thead class="bg-gray-50/80 text-gray-500 text-xs uppercase tracking-wider">
                <tr>
                    ...
                    <th class="px-6 py-4 font-semibold text-center">Số Bài Viết Liên Kết</th>
                ...
                @forelse($danhMucs as $item)
                    <tr class="hover:bg-gray-50/50 transition-colors {{ $item->is_delete ? 'bg-red-50/30' : '' }}">
                        ...
                        <td class="px-6 py-4 text-center text-blue-600 font-bold">
                            <a href="{{ route('admin.tin-tuc.index',['danh_muc_id' => $item->danh_muc_tin_tuc_id]) }}" class="hover:underline">{{ $item->tin_tucs_count }} bài viết</a>
                        </td>
    ...
    <div id="deleteModal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/60 backdrop-blur-sm px-4 opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center transform scale-95 transition-transform duration-300">
            ...
            <p class="text-sm text-gray-500 mb-6">Bạn có thể khôi phục lại bất cứ lúc nào. Các bài viết thuộc danh mục này KHÔNG bị ảnh hưởng hiển thị.</p>
