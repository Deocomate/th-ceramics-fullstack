<x-admin.layouts.app title="ThÃªm ÄÃ¨n VÆ°á»n Gá»‘m Sá»©" breadcrumb="Admin â€º DS Sáº£n pháº©m chi tiáº¿t â€º ThÃªm má»›i">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wide">ThÃ´ng tin dÃ¡ng ngÃ³i má»›i</h2>
        </div>
        <form method="POST" action="{{ route('admin.den-vuon-gom-su-ct.store') }}" enctype="multipart/form-data" class="p-6">
            @csrf
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <!-- Cá»˜T THÃ”NG TIN CHUNG (KhÃ´ng Code, KhÃ´ng GiÃ¡) -->
                <div class="lg:col-span-2 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="VD: ÄÃ¨n VÆ°á»n Gá»‘m Sá»© Cá»¡ Nhá»" class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
                            @error('name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <x-admin.shared.color-field />
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">NhÃ³m hiá»ƒn thá»‹ <span class="text-red-500">*</span></label>
                            <select name="category_type" required class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all bg-white">
                                <option value="den_gom" @selected(old('category_type', 'den_gom') === 'den_gom')>ÄÃ¨n Gá»‘m</option>
                                <option value="den_su" @selected(old('category_type') === 'den_su')>ÄÃ¨n Sá»©</option>
                            </select>
                    ...
                    <div class="bg-gray-50/80 rounded-xl border border-gray-200 p-5">
                        <div class="flex items-center justify-between mb-4 border-b border-gray-200 pb-3">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-0.5">Má»—i khá»‘i tÆ°Æ¡ng á»©ng vá»›i 1 gáº¡ch Ä‘áº§u dÃ²ng (bullet point) trÃªn Website.</p>
                            </div>
                    ...
                    <!-- BLOCKS CHI TIáº¾T KÃCH THÆ¯á»šC (SIZE DES) -->
                    <div class="bg-blue-50/30 rounded-xl border border-blue-100 p-5 mt-5">
                        <div class="flex items-center justify-between mb-4 border-b border-blue-200 pb-3">
                            <div>
                                <label class="block text-sm font-bold text-blue-800">Danh sÃ¡ch KÃ­ch thÆ°á»›c chi tiáº¿t (Náº¿u cÃ³)</label>
                            ...
                            <button type="button" onclick="addSizeDesBlock()" class="px-4 py-2 bg-white border border-gray-300 text-gray-700 text-xs font-bold rounded-lg hover:bg-blue-50 hover:text-blue-700 hover:border-blue-400 transition-colors shadow-sm flex items-center gap-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> ThÃªm dÃ²ng
                            </button>
                        </div>
                        <div id="size-des-blocks-container" class="space-y-2.5">
                            <!-- JS sáº½ render input size_des vÃ o Ä‘Ã¢y -->
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col h-full border border-gray-200 rounded-xl p-6 bg-gray-50/50">
                <label class="block text-sm font-semibold text-gray-700 mb-2">HÃ¬nh áº£nh dÃ¡ng sáº£n pháº©m chung <span class="text-red-500">*</span></label>
                <p class="text-xs text-gray-500 mb-4">Chá»n 1 hoáº·c nhiá»u áº£nh chi tiáº¿t chung cá»§a sáº£n pháº©m (HÃ¬nh há»c, gÃ³c cáº¡nh...). phÃ¢n loáº¡i sáº½ Ä‘Æ°á»£c quáº£n lÃ½ á»Ÿ pháº§n Biáº¿n Thá»ƒ.</p>
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.den-vuon-gom-su-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Há»§y</a>
    ...
    <script>
        ...
        function addDesBlock(value = '') {
            ...
            div.innerHTML = `
                ...
                <input type="text" name="des[]" value="${value}" placeholder="VD: Trá»ng lÆ°á»£ng: 1kg / viÃªn" class="flex-1 py-2.5 px-2 text-sm border-none focus:ring-0 outline-none text-gray-700 bg-transparent placeholder-gray-400">
        ...
        // ======= LOGIC CHO BLOCK SIZE DES (KÃCH THÆ¯á»šC CHI TIáº¾T) =======
        ...
        function addSizeDesBlock(value = '', autoFocus = true) {
            ...
            div.innerHTML = `
                ...
                <input type="text" name="size_des[]" value="${value}" placeholder="VD: Chiá»u dÃ i nÃ³c: 300mm" class="flex-1 py-2.5 px-2 text-sm border-none focus:ring-0 outline-none text-gray-700 bg-transparent placeholder-gray-400">
                <button type="button" onclick="this.parentElement.remove()" class="px-3 text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100" title="XÃ³a dÃ²ng nÃ y">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            `;
            ...
            // Tá»± Ä‘á»™ng focus vÃ o input vá»«a táº¡o náº¿u báº¥m nÃºt "ThÃªm dÃ²ng"
            if(autoFocus && value === '') {
                div.querySelector('input').focus();
            }
        }
        
        // Khá»Ÿi táº¡o sáºµn 1 dÃ²ng trá»‘ng ban Ä‘áº§u
        addSizeDesBlock('');
