<x-admin.layouts.app title="Thêm Đèn Vườn Gốm Sứ" breadcrumb="Admin › DS Sản phẩm chi tiết › Thêm mới">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.den-vuon-gom-su-ct.store') }}" enctype="multipart/form-data" class="p-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="VD: Đèn Vườn Gốm Sứ Cỡ Nhỏ" class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
                        ...
                        <div>
                            ...
                            <select name="category_type" required class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all bg-white">
                                <option value="den_gom" @selected(old('category_type', 'den_gom') === 'den_gom')>Đèn Gốm</option>
                                <option value="den_su" @selected(old('category_type') === 'den_su')>Đèn Sứ</option>
                            </select>
                    ...
                    <div class="bg-blue-50/30 rounded-xl border border-blue-100 p-5 mt-5">
                        ...
                        <div id="size-des-blocks-container" class="space-y-2.5">
                            <!-- JS sẽ render input size_des vào đây -->
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col h-full border border-gray-200 rounded-xl p-6 bg-gray-50/50">
                ...
                <p class="text-xs text-gray-500 mb-4">Chọn 1 hoặc nhiều ảnh chi tiết chung của sản phẩm (Hình học, góc cạnh...). phân loại sẽ được quản lý ở phần Biến Thể.</p>
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.den-vuon-gom-su-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Hủy</a>
    ...
    <script>
        ...
        function addDesBlock(value = '') {
            ...
            div.innerHTML = `
                ...
                <input type="text" name="des[]" value="${value}" placeholder="VD: Trọng lượng: 1kg / viên" class="flex-1 py-2.5 px-2 text-sm border-none focus:ring-0 outline-none text-gray-700 bg-transparent placeholder-gray-400">
        ...
        // ======= LOGIC CHO BLOCK SIZE DES (KÍCH THƯỚC CHI TIẾT) =======
        ...
        function addSizeDesBlock(value = '', autoFocus = true) {
            ...
            div.innerHTML = `
                ...
                <input type="text" name="size_des[]" value="${value}" placeholder="VD: Chiều dài nóc: 300mm" class="flex-1 py-2.5 px-2 text-sm border-none focus:ring-0 outline-none text-gray-700 bg-transparent placeholder-gray-400">
                <button type="button" onclick="this.parentElement.remove()" class="px-3 text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100" title="Xóa dòng này">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            `;
            ...
            // Tự động focus vào input vừa tạo nếu bấm nút "Thêm dòng"
            if(autoFocus && value === '') {
                div.querySelector('input').focus();
            }
        }
        
        // Khởi tạo sẵn 1 dòng trống ban đầu
        addSizeDesBlock('');
