...
<x-admin.layouts.app title="C?p nh?t Ðèn Vu?n G?m S?" breadcrumb="Admin › DS S?n ph?m chi ti?t › Ch?nh s?a">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.den-vuon-gom-su-ct.update', $product->den_vuon_gom_su_ct_id) }}" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Nhóm hi?n th? <span class="text-red-500">*</span></label>
                            <select name="category_type" required class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all bg-white">
                                <option value="den_gom" @selected(old('category_type', $product->category_type) === 'den_gom')>Ðèn G?m</option>
                                <option value="den_su" @selected(old('category_type', $product->category_type) === 'den_su')>Ðèn S?</option>
                            </select>
                    ...
                    <!-- BLOCKS CHI TI?T KÍCH THU?C (SIZE DES) -->
                    <div class="bg-blue-50/30 rounded-xl border border-blue-100 p-5 mt-5">
                        <div class="flex items-center justify-between mb-4 border-b border-blue-200 pb-3">
                            <div>
                                <label class="block text-sm font-bold text-blue-800">Danh sách Kích thu?c chi ti?t</label>
                                <p class="text-xs text-gray-500 mt-0.5">Hi?n th? thông s? chi ti?t bên c?nh ?nh B?n v?.</p>
                            </div>
                        ...
                        <div id="size-des-blocks-container" class="space-y-2.5">
                            <!-- JS s? render input size_des vào dây -->
                        </div>
                    </div>
                </div>
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.den-vuon-gom-su-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">H?y b?</a>
    ...
    @include('admin.partials.product-gallery-manager', [
        ...
        'destroyUrl' => route('admin.den-vuon-gom-su-ct.image.destroy', $product->den_vuon_gom_su_ct_id),
        'uploadUrl' => route('admin.den-vuon-gom-su-ct.image.store', $product->den_vuon_gom_su_ct_id),
        'reorderUrl' => route('admin.den-vuon-gom-su-ct.gallery.reorder', $product->den_vuon_gom_su_ct_id),
    ])
