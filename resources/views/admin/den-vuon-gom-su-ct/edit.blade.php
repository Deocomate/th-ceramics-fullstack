...
<x-admin.layouts.app title="Cáº­p nháº­t ÄÃ¨n VÆ°á»n Gá»‘m Sá»©" breadcrumb="Admin â€º DS Sáº£n pháº©m chi tiáº¿t â€º Chá»‰nh sá»­a">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.den-vuon-gom-su-ct.update', $product->den_vuon_gom_su_ct_id) }}" enctype="multipart/form-data" class="p-6">
            @csrf @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div>
                            ...
                            <select name="category_type" required class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all bg-white">
                                <option value="den_gom" @selected(old('category_type', $product->category_type) === 'den_gom')>ÄÃ¨n Gá»‘m</option>
                                <option value="den_su" @selected(old('category_type', $product->category_type) === 'den_su')>ÄÃ¨n Sá»©</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="size" value="{{ old('size', $product->size) }}" placeholder="VD: L200 x W200 x D20 mm" class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
                        </div>
                    </div>
            ...
            <div class="flex flex-col h-full border border-gray-200 rounded-xl p-6 bg-gray-50/50">
                <label class="block text-sm font-semibold text-gray-700 mb-2">ThÃªm HÃ¬nh áº¢nh Chung Má»›i</label>
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.den-vuon-gom-su-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Há»§y bá»</a>
    ...
    <!-- DANH SÃCH áº¢NH HIá»†N Táº I -->
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wide">HÃ¬nh áº£nh chung hiá»‡n táº¡i</h2>
        ...
        <div class="p-6">
            ...
            @else
                <p class="text-gray-500 text-sm text-center py-6">Sáº£n pháº©m nÃ y chÆ°a cÃ³ hÃ¬nh áº£nh chung nÃ o.</p>
            @endif
        </div>
    </div>

    {{-- MODAL XÃ“A áº¢NH HIá»†N Táº I --}}
    <div id="deleteImageModal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/60 backdrop-blur-sm px-4 opacity-0 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform scale-95 transition-transform duration-300 p-6 text-center">
            ...
            <p class="text-sm text-gray-500 mb-6">áº¢nh nÃ y sáº½ bá»‹ xÃ³a khá»i danh sÃ¡ch áº£nh chung cá»§a sáº£n pháº©m.</p>
            <div class="flex justify-center gap-3">
                ...
                <form id="deleteImageForm" method="POST" action="{{ route('admin.den-vuon-gom-su-ct.image.destroy', $product->den_vuon_gom_su_ct_id) }}" class="flex-1">
                    @csrf @method('DELETE')
