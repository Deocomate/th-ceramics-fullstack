...
<div x-data="{
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        ...
        <form method="POST" action="{{ route('admin.pages.factory.update') }}" enctype="multipart/form-data" class="p-6">
            @csrf @method('PUT')
            ...
            <nav class="flex flex-wrap gap-1 border-b border-gray-200 mb-6">
                <template x-for="tab in tabs" :key="tab">
                    <button type="button"
                        ...
                        class="px-5 py-3 text-sm transition-colors duration-200"
            ...
            <div x-cloak x-show="activeTab === 'intro'" class="space-y-8">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Hỗ trợ HTML: &lt;b&gt;in đậm&lt;/b&gt;, &lt;br&gt; xuống dòng</p>
                    @error('intro_description') <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>

            {{-- ==================== TAB 3: GALLERY 1 ==================== --}}
            <div x-cloak x-show="activeTab === 'gallery'" class="space-y-8" x-data="{ images: {{ Js::from($factoryJson['gallery_1']) }}, deletedIndices: [] }">
                    ...
                    <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4">
                        <template x-for="(img, index) in images" :key="index">
                            <div class="relative group aspect-square rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-gray-100"
                                ...
                                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-200
            ...
            <div x-cloak x-show="activeTab === 'process'" class="space-y-8"
                x-data="{ images: {{ Js::from($factoryJson['process_slider']) }}, deletedIndices: [] }">
                ...
                <div>
                    <label for="process_description" class="block text-sm font-semibold text-gray-700 mb-2">Mô tả (Hỗ trợ HTML)</label>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Hỗ trợ HTML: &lt;b&gt;in đậm&lt;/b&gt;, &lt;br&gt; xuống dòng, &lt;ul&gt;&lt;li&gt;danh sách&lt;/li&gt;&lt;/ul&gt;</p>
                ...
                <div>
                    ...
                    <textarea id="process_bottom_desc" name="process_bottom_desc"
                        x-data
                        x-init="$el.style.height = $el.scrollHeight + 'px'"
                        @input="$el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px'"
                        class="w-full px-4 py-2.5 text-sm border rounded-lg outline-none transition-all
            ...
            <div x-cloak x-show="activeTab === 'material'" class="space-y-8"
                x-data="{ images: {{ Js::from($factoryJson['material_slider']) }}, deletedIndices: [], steps: {{ Js::from($factoryJson['material_steps']) }} }">
                ...
                <div>
                            ...
                            <div class="border border-gray-200 rounded-xl p-5 bg-gray-50/80 hover:bg-gray-50 transition-colors shadow-sm"
                                ...
                                <div class="flex items-center justify-between mb-4">
                                    <div class="flex items-center gap-3">
                                        <span class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white bg-brand-red"
                                            x-text="index + 1"></span>
                                        <span class="text-sm font-semibold text-gray-700">Bước <span x-text="step.number"></span></span>
                                    </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Số thứ tự</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Tiêu đề</label>
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Mô tả</label>
                    ...
                    <p x-show="steps.length === 0" class="text-sm text-gray-400 text-center py-6 rounded-lg border border-dashed border-gray-300 mt-4">
                        Chưa có bước nguyên liệu nào. Nhấn "Thêm bước" để bắt đầu.
                    </p>

                    <button type="button"
                        @click="
                            steps.push({number: String(steps.length + 1), title: '', description: ''});
