<x-admin.layouts.app title="ThÃªm Linh Váº­t Phong Thá»§y" breadcrumb="Admin â€º DS Sáº£n pháº©m chi tiáº¿t â€º ThÃªm má»›i">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-8">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h2 class="text-sm font-bold text-gray-800 uppercase tracking-wide">ThÃ´ng tin Linh váº­t má»›i</h2>
        </div>
        <form method="POST" action="{{ route('admin.linh-vat-phong-thuy-ct.store') }}" enctype="multipart/form-data" class="p-6">
                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="size" value="{{ old('size') }}" placeholder="VD: Cao 30cm, Rá»™ng 15cm" class="w-full px-4 py-2.5 text-sm border rounded-lg border-gray-300 focus:border-[#A31D1D] focus:ring-1 focus:ring-[#A31D1D] outline-none transition-all">
                        </div>
                    </div>

                    <!-- BLOCKS THÃ”NG Sá» CHUNG (DES) -->
                    <div class="bg-gray-50/80 rounded-xl border border-gray-200 p-5">
                        <div class="flex items-center justify-between mb-4 border-b border-gray-200 pb-3">
                            <div>
                                <label class="block text-sm font-bold text-gray-800">ThÃ´ng sá»‘ / Ã nghÄ©a phong thá»§y</label>
                    ...
                    <div class="bg-blue-50/30 rounded-xl border border-blue-100 p-5">
                        <div class="flex items-center justify-between mb-4 border-b border-blue-200 pb-3">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-0.5">Hiá»ƒn thá»‹ cáº¡nh áº£nh KÃ­ch thÆ°á»›c/Báº£n váº½ bÃªn dÆ°á»›i.</p>
                            </div>
            ...
            <div class="pt-6 mt-8 flex justify-end gap-3 border-t border-gray-100">
                <a href="{{ route('admin.linh-vat-phong-thuy-ct.index') }}" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Há»§y</a>
    ...
    @push('scripts')
        <script>
            ...
            function addDesBlock(value = '', autoFocus = true) {
                ...
                div.innerHTML = `
                    <input type="text" name="des[]" value="${value}" placeholder="VD: Mang Ã½ nghÄ©a chiÃªu tÃ i..." class="flex-1 py-2.5 px-3 text-sm border-none focus:ring-0 outline-none text-gray-700 bg-transparent">
            ...
            function addSizeDesBlock(value = '', autoFocus = true) {
                ...
                div.innerHTML = `
                    <input type="text" name="size_des[]" value="${value}" placeholder="VD: ÄÆ°á»ng kÃ­nh Ä‘áº¿: 10cm" class="flex-1 py-2.5 px-3 text-sm border-none focus:ring-0 outline-none text-gray-700 bg-transparent">
                    <button type="button" onclick="this.parentElement.remove()" class="px-3 text-red-400 hover:text-red-600">X</button>
                `;
            ...
            // Logic Upload nhiá»u áº£nh (Giá»‘ng há»‡t cÃ¡c file khÃ¡c, báº¡n cÃ³ thá»ƒ copy function handleMultipleFiles...)
            let selectedFiles =[];
                        ...
                        reader.onload = function(e) {
                            ...
                            div.innerHTML = `<img src="${e.target.result}" class="w-full h-full object-contain">
                                             <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center">
                                                <button type="button" onclick="removeFile(${index})" class="text-white text-xl">Ã—</button>
                                             </div>`;
